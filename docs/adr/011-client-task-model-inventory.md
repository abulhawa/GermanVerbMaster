...
| --- | --- | --- | --- |
| Verb data helpers | `client/src/lib/verbs.ts` | Fetches `/api/quiz/verbs`, caches verb seed bundles, and normalises results into `GermanVerb`. All downstream callers expect verb fields like `infinitive`, `präteritum`, `partizipII`, and the `auxiliary` flag. | Supersede with a task fetch module that loads `/api/tasks`, validates payloads with the shared registry, and exposes verb tasks as `PracticeTask` entries so new POS task types share the same entry point. |
