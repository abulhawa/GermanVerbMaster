...
3. Apply the latest migrations to your Postgres database:
   ...
   npm run db:push
   ```
4. The legacy seed pipeline has been retired. The `npm run seed` stub remains for compatibility but prints a notice instead of mutating the database.
...
6. Additional scripts:
   ...
   npm run db:reset   # drop the enrichment schema and clear generated data (preserves data/pos)
   ```
...
Before running `npm run db:push` (or invoking the legacy `npm run seed` stub) in any managed environment, set the same variables documented in `.env.example`:
...
These must be present both in Vercel’s Environment Variables UI and in any CI job that invokes the migration (`npm run db:push`) script so the Drizzle client connects with the correct SSL options.
...
On Vercel, configure the project to run `npm install`, `npm run db:push`, and `npm run build` as the build step. Point the “Output Directory” at `dist/public` so static assets are uploaded automatically, and map `/api/*` routes to the bundled handler (see `server/api/vercel-handler.ts`) via the existing `vercel.json` configuration.
...
- For manual verification against a live Postgres instance, point `DATABASE_URL` at your sandbox (see above) and run `npm run db:push` before hitting the API through the Vercel handler or Express dev server. The legacy seeding CLI is a no-op because enrichment fixtures now ship via migrations.

## Release checklist

1. `npm run db:push`
2. `npm run test:all`
3. Deploy the build and confirm the Vercel Cron job for `/api/jobs/regenerate-queues` is still scheduled and returning `200`.
...
- Legacy aggregated CSVs now live under `data/legacy/` for historical reference. POS-specific JSONL files under `data/pos/` remain the source of truth for deterministic task packs.
...
- The legacy `npm run seed` CLI no longer mutates data; enrichment tables are provisioned through migrations and manual pack curation. Keep `data/packs/*.json` in sync with `data/pos/*.jsonl` as part of your release process.
- Historical seeding notes remain for reference, but live deployments now rely on Drizzle migrations plus the enrichment export/import helpers documented under `scripts/enrichment/`.
...
- To manually import curated enrichments, prepare a JSON file that matches the schema in [`docs/enrichment-import-template.json`](docs/enrichment-import-template.json) and run `npm run enrichment:import -- --file path/to/import.json`. Optional flags: `--provider-id`, `--provider-label`, `--mode` (`approved`, `pending`, or `all`), and `--skip-backups` to suppress the `words` backup. The script validates each entry with Zod, updates the `words` table, stores an `enrichment_provider_snapshots` row with `trigger="apply"`, and rewrites the corresponding `data/enrichment/<pos>/<provider>.json` file so the change is ready for commit. By default entries merge into existing translations/examples, so you can supply partial updates (for example, extra bilingual examples only) without overwriting other fields. Set `applyMode: "replace"` at the file root or per-entry when you explicitly want to overwrite the stored values instead of merging.
